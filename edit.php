...
<head>
    ...
    <style>
        ...
        #image-preview { max-width: 200px; max-height: 200px; margin-top: 10px; border: 1px solid var(--neutral-200); border-radius: var(--radius); display: <?= !empty($item['image']) ? 'block' : 'none' ?>; }
...
<div class="admin-container">
    ...
    <form method="POST" id="menu-form">
        ...
        <div class="form-group">
            ...
            <input type="url" name="image" id="image-url" value="<?= htmlspecialchars($item['image'] ?? '') ?>" placeholder="https://example.com/image.jpg">
            <div class="small-note">Provide a direct link to an image. Leave blank to keep current (or remove URL to delete image).</div>
            <img id="image-preview" src="<?= htmlspecialchars($item['image'] ?? '') ?>" alt="Image preview">
        </div>
